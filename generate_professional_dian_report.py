#!/usr/bin/env python3
"""
Script para generar reporte profesional de conceptos DIAN 
en el formato exacto requerido con resúmenes y análisis detallados.
"""
...
def generate_professional_dian_report():
    ...
    # Save report
    report_content = "\n".join(report_lines)
    report_file = f"dian_conceptos_profesional_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    print("=" * 70)
    print(report_content[:3000] + "..." if len(report_content) > 3000 else report_content)
    
    return report_file

if __name__ == "__main__":
    generate_professional_dian_report()